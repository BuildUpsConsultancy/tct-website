...
const CinematicGallery = () => {
  ...
  // Smooth snap to a target progress value (used on touchend)
  ...
  // Nudge progress by a raw pixel delta (positive = scroll down)
  const nudgeProgress = useCallback((delta: number) => {
    ...
    if (rafRef.current && !animatingRef.current) cancelAnimationFrame(rafRef.current);
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      e.preventDefault();
      nudgeProgress(dy * 2.2);
      touchStartY.current = e.touches[0].clientY;
    };

    const onTouchEnd = () => {
      const p = progressRef.current;
      if (p < 0.06) smoothTo(0);
      else if (p > 0.94) smoothTo(1);
    };
